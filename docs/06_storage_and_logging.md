...
| --- | --- | --- |
| MCU internal Flash | Firmware, configuration, calibration | Separate from record storage (`PR-STORAGE-007`). |
| External SPI NOR | Measurements, events, faults, buffered records | Append-heavy record storage. |
...
Buffered records are released only after upload confirmation
(`PR-STORAGE-008`).

### 9.3 No silent loss

```text
record created -> stored locally -> uploaded -> confirmed -> released
```
...
| Communication recovery | Records synchronized and uploaded, then released. |
