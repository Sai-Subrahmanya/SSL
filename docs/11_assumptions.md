...
### A-18 - Relay feedback is available from the switching element

| Field | Value |
| --- | --- |
| Assumption | The switching element provides relay feedback that can be read by the lamp node. |
| Reason | Expected-versus-actual diagnosis requires the switching element state to be observable separately from the command. |
| Status | `Open` |
| Effect if changed | If feedback is unavailable, `PR-DIAG-003` and `PR-DIAG-004` lose a primary evidence input and the diagnostic rules must be revised. This is the highest-impact assumption in the diagnostics area. |
...
| `Open` | 16 | A-05, A-07, A-08, A-09, A-10, A-11, A-12, A-13, A-14, A-15, A-16, A-17, A-18, A-19, A-20, A-21 |
| `Accepted` | 6 | A-04, A-06, A-22, A-23, A-24, A-25 |
| **Total** | **25** | A-01 .. A-25 |
