...
3. **Explicit state.** Every element exposes state (commanded, switching
   feedback, actual, sensor, communication, controller) rather than
   inferring health from a single measurement. "Switching feedback" is an
   abstraction; its physical realisation is a hardware-design decision.
...
| 3 | Switching feedback | Reporting the observed state of the switching path separately from the command. Physical implementation is a hardware-design decision. |
...
| 16 | Local storage / buffer | Abstract local storage for node data, communication events, configuration transactions, synchronization state and upstream-failure buffering. Physical memory device is deliberately not selected. |
...
### 5.4 Group Controller local storage (abstract)

The Group Controller requires an **abstract local storage/buffer** capability
because it may need to buffer:

- node data (measurements, status, faults, events),
- communication events,
- configuration transactions,
- synchronization state,
- data that could not be forwarded during an upstream communication failure.

The physical implementation (internal MCU flash, external memory, or another
medium) is **not selected** at this stage and is deferred to the hardware
architecture phase. No component is implied by this requirement.
